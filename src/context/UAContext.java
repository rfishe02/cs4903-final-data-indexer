	...
	public boolean buildTermContextMatrix(File[] files, String outputDirectory, int windowSize) {
		try {
			if (new File(outputDirectory).exists()) {
				...
				BufferedWriter logs = new BufferedWriter(new OutputStreamWriter(
						new FileOutputStream(outputDirectory + "/errors.log"), StandardCharsets.UTF_8));
...
//		System.out.println(context.termContextRecordSize);
		UAContext context = new UAContext(Integer.parseInt(args[0]));
		File[] files = new File(args[1]).listFiles();
		context.buildTermContextMatrix(files, args[2], Integer.parseInt(args[3]));
	}
